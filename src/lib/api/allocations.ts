...
export async function cancelAllocation(id: string): Promise<any> {
  const result = await db.query('SELECT * FROM cancel_allocation($1)', [id]);
  return result.rows[0];
}

export async function getAllocationHistory(allocationId: string): Promise<any[]> {
  const result = await db.query('SELECT * FROM allocation_history WHERE allocation_id = $1 ORDER BY created_at DESC', [allocationId]);
  return result.rows;
}
...
export async function findCompatibleWorkspaces(
  ...
  const workspacesResult = await db.query('SELECT * FROM workspaces');
  const workspaces = workspacesResult.rows;

  const roomsResult = await db.query('SELECT * FROM rooms');
  const rooms = roomsResult.rows;

  const existingAllocationsResult = await db.query('SELECT * FROM allocations WHERE status IN ($1, $2)', ['active', 'pending']);
  ...
  return compatibleWorkspaces.map(({ workspace, score }) => ({
    ...
    name: workspace.name, 
    room_name: rooms.find(r => r.id === workspace.roomId)?.name || '',
    compatibility_score: score
  }));
}

export async function autoAllocateSeats(): Promise<any[]> {
  const employeesResult = await db.query('SELECT * FROM employees WHERE status = $1', ['active']);
  const employees = employeesResult.rows;

  const workspacesResult = await db.query('SELECT * FROM workspaces WHERE status = $1', ['available']);
  const workspaces = workspacesResult.rows;
...
export async function validateAllocation(
  ...
  try {
    ...
    const workspaceResult = await db.query('SELECT * FROM workspaces WHERE id = $1', [workspaceId]);
    ...
    const roomResult = await db.query('SELECT * FROM rooms WHERE id = $1', [workspace.room_id]);
